                    ...
                    if (_CurrentLoot == null)
                    {
                        ...
                        for (var i = 0; i < ret.Count; i++)
                        {
                            ...
                            if (_OverrideLoot.ContainsKey(element.spawnableItem.itemName))
                            {
                                ...
                                ret[i] = new SpawnableItemWithRarity(element.spawnableItem, rarity);
                            }
                        }
                        foreach (var e in _OverrideLoot)
                        {
                            if (!foundScrap.Contains(e.Key))
                            {
                                var rarity = e.Value;
                                if (rarity > 0)
                                    ret.Add(new SpawnableItemWithRarity(Items.ScrapItemsByName[e.Key],rarity));
                            }
                        }
                        _CurrentLoot = ret;
                    }
                    ...
                    if (_CurrentDaytimeEnemies == null)
                    {
                        ...
                        for (var i = 0; i < ret.Count; i++)
                        {
                            ...
                            if (_OverrideDaytimeEnemies.ContainsKey(element.enemyType.enemyName))
                            {
                                ...
                                ret[i] = new SpawnableEnemyWithRarity(element.enemyType, rarity);
                            }
                        }
                        foreach (var e in _OverrideDaytimeEnemies)
                        {
                            if (!foundEnemies.Contains(e.Key))
                            {
                                var rarity = e.Value;
                                if (rarity > 0)
                                    ret.Add(new SpawnableEnemyWithRarity(Enemies.EnemiesByName[e.Key].EnemyType, rarity));
                            }
                        }
