                ...
                if (item.Value.IsScrap)
                {
                    if (item.Value.LimitPlanets)
                    {
                        for (var i = 0; i < Game.Moons.Count; i++)
                        {
                            ...
                            {
                                Moons[i].spawnableScrap.Add(new SpawnableItemWithRarity(Game.Manager.ItemProperties[item.Value.ID], item.Value.PlanetRarities[i]));
                            }
                        }
                    }
                    else
                    {
                        foreach (var m in Moons)
                            m.Value.spawnableScrap.Add(new SpawnableItemWithRarity(Game.Manager.ItemProperties[item.Value.ID], item.Value.Rarity));
                    }
                }
            }
        }
